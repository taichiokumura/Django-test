...
# マーカーの設定
marker=cv2.imread('image/marker2.png',0) 
w, h = marker.shape[::-1]

# スキャン画像を読み込む
img = cv2.imread('image/sheet5.png',0)
...
cv2.imwrite('image/res2.png',img)
...
img = 255 - img
cv2.imwrite('image/sirokuro2.png',img)
...
for x, row_result in enumerate(result):
    ...
    if len(marked_answer_indices) > 0:
        ...
        if len(marked_answers) > 1:
            print('Q%d: ' % (x+1) + ' / '.join(marked_answers) + ' ## 複数回答 ##')
        else:
            print('Q%d: ' % (x+1) + marked_answers[0])
    else:
        # マークがない場合
        print('Q%d: ** 未回答 **' % (x+1))
